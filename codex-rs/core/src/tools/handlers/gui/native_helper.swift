...
enum HelperError: Error, CustomStringConvertible {
    ...
    var description: String {
        ...
        case .screenshotFailed(let detail):
            return "screenshotFailed(\(detail))"
        case .fileReadFailed(let detail):
            return "fileReadFailed(\(detail))"
        }
    }
}
...
func loadWindowExclusions() -> WindowExclusions {
    let ownerNames = mergeNormalizedValues(
        parseDelimitedEnv("CODEX_GUI_EXCLUDED_OWNER_NAMES")
            + parseDelimitedEnv("CODEX_GUI_AUTO_EXCLUDED_OWNER_NAMES")
    )
    let bundleIds = mergeNormalizedValues(
        parseDelimitedEnv("CODEX_GUI_EXCLUDED_BUNDLE_IDS")
            + parseDelimitedEnv("CODEX_GUI_AUTO_EXCLUDED_BUNDLE_IDS")
    )
...
func preferWindowWhenAvailable() -> Bool {
    env("CODEX_GUI_PREFER_WINDOW") == "1"
}

func requestedCaptureMode() throws -> String? {
    guard let value = trimmedEnv("CODEX_GUI_CAPTURE_MODE") else {
        return nil
    }
    switch value {
    case "display", "window":
        return value
    default:
        throw HelperError.invalidEnv("CODEX_GUI_CAPTURE_MODE")
    }
}
...
func captureScreenshot(bounds: CGRect) throws -> String {
    let imagePath = (NSTemporaryDirectory() as NSString).appendingPathComponent(
        "codex-gui-observe-\(UUID().uuidString).png"
    )
    let region = [
    ...
    process.arguments = ["-x", "-C", "-R", region, imagePath]
    let stderrPipe = Pipe()
    ...
    guard process.terminationStatus == 0 else {
        ...
        throw HelperError.screenshotFailed(stderr.isEmpty ? "screencapture_exit_\(process.terminationStatus)" : stderr)
    }
    guard FileManager.default.fileExists(atPath: imagePath) else {
        throw HelperError.fileReadFailed("missing_capture_\(imagePath)")
    }
    return imagePath
}
...
func redactImageAtPath(_ imagePath: String, redactions: [CGRect]) throws {
...
func handleRedactHostWindows() throws {
    let imagePath = try requiredPath("CODEX_GUI_IMAGE_PATH")
    let captureBounds = CGRect(
        x: try requiredDouble("CODEX_GUI_CAPTURE_X"),
        y: try requiredDouble("CODEX_GUI_CAPTURE_Y"),
        width: try requiredDouble("CODEX_GUI_CAPTURE_WIDTH"),
        height: try requiredDouble("CODEX_GUI_CAPTURE_HEIGHT")
    )
    let exclusions = loadWindowExclusions()
...
func handleObserve() throws {
    let requestedApp = trimmedEnv("CODEX_GUI_APP")
    let requestedWindowTitle = trimmedEnv("CODEX_GUI_WINDOW_TITLE")
    let requestedWindowTitleContains = trimmedEnv("CODEX_GUI_WINDOW_TITLE_CONTAINS")
    let requestedWindowIndex = optionalInt("CODEX_GUI_WINDOW_INDEX")
    let requestedMode = try requestedCaptureMode()
    let preferWindow = preferWindowWhenAvailable()
    let exclusions = loadWindowExclusions()
    let frontmostApp = NSWorkspace.shared.frontmostApplication
    let hostFrontmostExcluded = matchesExcludedApplication(frontmostApp, exclusions: exclusions)
    let windowSelectionRequested =
        normalizedText(requestedWindowTitle) != nil ||
        normalizedText(requestedWindowTitleContains) != nil ||
        requestedWindowIndex != nil
    if shouldActivateApp() {
        try activateApplication(named: requestedApp)
    }
    ...
    if windowSelectionRequested && window == nil {
        throw HelperError.applicationNotFound("requested_window")
    }
    ...
    default:
        useWindow =
            ...
            || (
                !preferWindow
                && !exclusions.isEmpty
                && hostFrontmostExcluded
                && window != nil
            )
            || (preferWindow && window != nil)
    }
    guard requestedMode != "window" || window != nil else {
        throw HelperError.invalidEnv("CODEX_GUI_CAPTURE_MODE")
    }

    let captureMode = useWindow ? "window" : "display"
    let captureBounds = useWindow ? window!.captureBounds.integral : display.bounds.integral
    let captureWidth = Int(captureBounds.width.rounded())
    let captureHeight = Int(captureBounds.height.rounded())
    let imagePath = try captureScreenshot(bounds: captureBounds)
    let payload = ObservePayload(
        appName: targetApp,
        display: DisplayDescriptor(index: display.index, bounds: rect(display.bounds)),
        cursor: point(cursorLocation),
        windowId: useWindow ? window?.primary.id : nil,
        windowTitle: useWindow ? window?.primary.title : nil,
        windowBounds: useWindow ? window.map { rect($0.captureBounds) } : nil,
        windowCount: useWindow ? window?.windowCount : nil,
        windowCaptureStrategy: useWindow ? window?.captureStrategy : nil,
        hostSelfExcludeApplied: !exclusions.isEmpty,
        hostFrontmostExcluded: hostFrontmostExcluded,
        hostFrontmostAppName: frontmostApp?.localizedName,
        hostFrontmostBundleId: frontmostApp?.bundleIdentifier,
        captureMode: captureMode,
        captureBounds: rect(captureBounds),
...
func handleEvent() throws {
    ...
    case "type_text":
        ...
        let shouldReplace = env("CODEX_GUI_REPLACE") != "0"
        let shouldSubmit = env("CODEX_GUI_SUBMIT") == "1"
        let typeStrategy = trimmedEnv("CODEX_GUI_TYPE_STRATEGY") ?? "unicode"
        let clearRepeat = max(0, optionalInt("CODEX_GUI_CLEAR_REPEAT") ?? (typeStrategy == "clipboard_paste" ? 48 : 0))
        if shouldReplace {
            if typeStrategy == "clipboard_paste" || typeStrategy == "physical_keys" {
...
func handleCleanup() throws {
    if env("CODEX_GUI_RELEASE_MOUSE") != "0" {
        releaseMouseButtons()
    }
    if env("CODEX_GUI_RELEASE_MODIFIERS") != "0" {
